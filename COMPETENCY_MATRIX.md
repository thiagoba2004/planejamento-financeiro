# Matriz de Competências — PRJ-000004
...
| **ADERÊNCIA PÚBLICA AO CFP®** | COMPETÊNCIA/CONCEITO DO DOMÍNIO — Módulo II Gestão Financeira confirmado oficialmente; conteúdo específico de débitos/créditos, sobre-endividamento, medidas corretivas e índices permanece em evidência Grau B até leitura direta integral da fonte oficial |
...
4. A comunicação pública deve usar a taxonomia `mapeado diretamente` / `competência do domínio` / `aplicação profissional do Projeto`; nomenclaturas internas não autorizam afirmar que um conteúdo específico “cai na prova”.
5. Aprovação em prova e competência profissional são dimensões distintas.
6. Questão objetiva isolada não comprova competência de diagnóstico, comunicação ou julgamento.
7. Toda elevação de Grau B para Grau A deve ser registrada sem apagar o histórico.
